...
# TODO: Double-Check Buy vs Low_10 vs purchased, vs BOUGHT -  
# set edit thresholds - create variables? (TL vs SL) : .75 tp or sl delta - 

bought_price = None 

token = 'CXT'               # TODO: 
buy_price = .06675              # TODO: Enter Desired Initial Buy Price

stop_loss_value = .10         # TODO: slv * 100 = Sl percent
take_profit_value = .015       # TODO: tpv * 100 = TP percent
min_order_multplier = 5         # TODO: Purchase amount - min_order * momo

# Gather token data
token_pair = f'{token}USD'

# Get Token Info Data - (used for orders decimal rounding, order mins, etc.)
...
# Calculations - get Asset data: Volume & Price max decimals allowed 
asset_data = get_asset_data(token_pair)
order_min = float(asset_data[3])                # Decimals for Order Min
order_calc = min_order_multplier * order_min
order_volume = order_calc       # NOTE: Order Vol = Min + 25%
price_round_value = asset_data[0]           # Decimals for Buy Price
volume_round_value = asset_data[2]          # Decimals for Volume


#volume_calc = 10 / buy_price       # Can be used to provide $$ value for Buy order Qt.  
...
def id_matching_order(token_pair, orders):
    ...
    for order_id, order_details in orders['open'].items(): 
        ...
        if asset_pair == token_pair:
            ...
            print(Fore.YELLOW + f'Order ID = {order_id} found for {token_pair}: Order Action = {order_action}, Order Price = {order_price}'+ Style.RESET_ALL)
...
def edit_open_order(token_pair, orders, edit_price):
    for order_id, order_details in orders['open'].items():
        asset_pair = order_details['descr']['pair']
        if asset_pair == token_pair:
            print(Fore.RED + f'Revising Order ID {order_id}, for Token: {token_pair}.' + Style.RESET_ALL)
...
def sell_check(trigger_price, limit_price, current_price, orders, buy_price, token_balance, stop_loss, stop_loss_delta, take_profit, take_profit_delta):
    ...
    tp_edit_delta = 0.5 * take_profit_delta
    tp_threshold = buy_price + tp_edit_delta
    sl_edit_delta = 0.9 * stop_loss_delta
    ...
    if current_price < sl_threshold:
        print(Fore.CYAN + f'SL Threshold Reached @ {sl_threshold} Current Price = {current_price}.Assure that STOP LOSS price is on ORDER' + Style.RESET_ALL)
